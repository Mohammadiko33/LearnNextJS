...
### با این روش که استفاده شده میشه از سرور کامپوننت توی کلاینت کامپوننت استفاده کنیم 

### با چیلدرن فرزنش رو پاس بدیم بهش

### نکته مهم : اگر یک کلاینت کامپوننت ، کامپوننت مارو رندر کنه اون کامپوننت تبدیل میشه به چی  کلاینت کامپوننت ، کدوم کامپوننت داره سرور کامپوننت رو رندر میکنه ، سرور کامپوننت 

### به عنوان چیلدرن میره داخلش میشینه در واقع این کامپوننت ابوت هست که داره صدا میکنه 

### و اگر یک سرور کامپوننت یک سرور کامپوننت رو رندر کنه اون کامپوننت یک سرور کامپوننت خواهد ماند

---
---

# Episode 25 : cache: "force-cache / no store"

### در ترمینال دوم جیسوم سرور رو ران کنید
...
### زمانی که داده ما استاتیک یا ثابت هست و تغییری انچنان زیادی نداره در زمان بیلد فقط ممکنه تغییری بکنه باید بیایم فورس کش بزاریم

#### فقط برای زمان هایی که داده قرار نیست زیاد تغییر کنه زمان های بیلد میخایم دیتا رو دوباره فچ کنیم 
...
### اگه میخاد هر بار فچ کنیم و تغییرات رو همزمان ببینیم بدون استور بزاریم

#### مثلا تو میخای روزی به تور مثال ده تا مقاله بزاری و نمیخای هر روز بیلد بگیری میایم میزاریم بدون استور که هر بار بیا درخاست بزنن
...
# Episode 26 : next { revalidate: 60 }

### این تکه خط کد میگه هر شصت ثانیه یک بار یک ریکوعست بزن و دیتا رو اپدیت کن ترکیبی از نو استور و فورس کش هست
...
### تا 60 ثانیه این ای پی ای کش میمونه و ده ثانیه بعد ما هرچقدر رفرش بزنیم تغییری صورت نمیگیره ولی بعد از 60 ثانیه مجدد ریکوعست میزنه دیتا رو دوباره میگیره

---
---

# Episode 29 : 'server-only' 

### این کتاب خونه اجازه میده ما از یک فایل از از فانکشن هاش اگزپورت گرفتیم فقط توی کامپوننت های سرور کامپوننت استفاده کنیم از این فانکشن ها

---
---

# Episode 30 : 'client-only' 

### دقیقا برعکس قبلی عمل میکنه میگه باید این فانکشن توی این فایل توی کلاینت کامپوننت استفاده بشه

---
---

# Episode 31 : Middleware

## 📌 Middleware چیه؟

### Middleware توی Next.js یه فانکشنیه که **قبل از اینکه یک ریکوئست کامل به سرور بره یا پاسخ داده بشه، اجرا میشه**.  

### یعنی قبل از لود شدن صفحه یا API، ما می‌تونیم یه چک انجام بدیم، کاربر رو ریدایرکت کنیم، یا محدودیت بذاریم.

---

## 🧠 کاربردهای Middleware

- بررسی احراز هویت (مثلاً اگر لاگین نکرده بود، به `/login` منتقلش کن)
- ریدایرکت کردن کاربر به صفحات خاص
- ست کردن زبان یا تم
- فیلتر کردن یا جلوگیری از دسترسی به بعضی مسیرها

---

## 🔧 نحوه نوشتن Middleware
...
* `request.url` کل آدرس URL فعلی هست.
* `new URL("/home", request.url)` یک URL جدید می‌سازه با مسیر جدید `/home` اما براساس ریشه URL اصلی.

مثال ساده‌تر:

```tsx
new URL("/home", "http://localhost:3000/about")
// نتیجه: http://localhost:3000/home
```

---

## ⚙️ محدود کردن اجرای Middleware به صفحات خاص

به‌صورت پیش‌فرض، Middleware روی **همه صفحات** اجرا میشه.
اما می‌تونیم محدودش کنیم با استفاده از `config.matcher`:
...
این یعنی Middleware فقط برای مسیر `/about` و زیرمسیرهاش اجرا میشه (مثل `/about/team` و ...)
...
فقط صفحه `/about` رو شامل میشه.

---

## ❓ اگه `config` نذاریم چی میشه؟

Middleware روی **تمام صفحات و API‌ها** اجرا میشه. برای همین حتما باید مشخص کنی می‌خوای کجا اجرا بشه، مخصوصاً وقتی می‌خوای فقط برای احراز هویت باشه.

---

## 🧪 مثال کاربردی: جلوگیری از دیدن صفحه Dashboard بدون لاگین
...
* Middleware قبل از اینکه صفحه لود بشه اجرا میشه
* برای بررسی auth، ریدایرکت و تغییر مسیر بسیار مفیده
* با `config.matcher` می‌تونی مشخص کنی کجا اجرا بشه
* `new URL("/page", request.url)` برای ساخت مسیر جدیده

---
---

# Episode 32 : context in next 

### نکته مهم : کانتکست رو هم باید توی کلاینت کامپوننت استفاده کنی حتما